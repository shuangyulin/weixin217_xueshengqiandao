<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="加入编号" prop="jiarubianhao">
            <el-input v-model="ruleForm.jiarubianhao" 
                placeholder="加入编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.jiarubianhao" label="加入编号" prop="jiarubianhao">
              <el-input v-model="ruleForm.jiarubianhao" 
                placeholder="加入编号" readonly></el-input>
          </el-form-item>
        </div>
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `jiarubanke/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.jiarubankeCrossAddOrUpdateFlag = false;
						 this.parent.search();

<template>
  <div class="addEdit-block">
    ...
    >
      <el-row >
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'" label="审批编号" prop="shenpibianhao">
            <el-input v-model="ruleForm.shenpibianhao" 
                placeholder="审批编号" readonly></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.shenpibianhao" label="审批编号" prop="shenpibianhao">
              <el-input v-model="ruleForm.shenpibianhao" 
                placeholder="审批编号" readonly></el-input>
          </el-form-item>
        </div>
      ...
      <el-col :span="12">
        ...
        <div v-else>
          <el-form-item class="input" label="审批状态" prop="shenpizhuangtai">
      ...
      <el-col :span="12">
        <el-form-item class="date" v-if="type!='info'" label="审批时间" prop="shenpishijian">
            <el-date-picker
                format="yyyy 年 MM 月 dd 日"
                value-format="yyyy-MM-dd"
                v-model="ruleForm.shenpishijian" 
                type="date"
                :readonly="ro.shenpishijian"
                placeholder="审批时间">
            </el-date-picker> 
        </el-form-item>
        <div v-else>
          <el-form-item class="input" v-if="ruleForm.shenpishijian" label="审批时间" prop="shenpishijian">
              <el-input v-model="ruleForm.shenpishijian" 
                placeholder="审批时间" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
...
export default {
  ...
  created() {
	this.ruleForm.shenpishijian = this.getCurDate()
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
        ...
        for (var o in obj){
          if(o=='shenpibianhao'){
            this.ruleForm.shenpibianhao = obj[o];
	    this.ro.shenpibianhao = true;
            continue;
          }
          if(o=='biaoti'){
      ...
      this.$http({
		...
		if(json.jiaoshixingming!=''&&json.jiaoshixingming){
          ...
          this.$message.error(data.msg);
            ...
            this.shenpizhuangtaiOptions = "有待审批,已请假,已完成,待销假".split(',')
    },
    // 多级联动参数
    info(id) {
      this.$http({
        url: `shenpixinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `shenpixinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.shenpixinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
